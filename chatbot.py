# --------------------------------------------------------
# CheemsChat
# Written by Shoufa Chen and Peize Sun based on visual-chatgpt (https://github.com/microsoft/visual-chatgpt)
# --------------------------------------------------------
...
CHEEMS_CHAT_PREFIX = """CheemsChat is developed by HKU and designed to be able to assist with a wide range of text and visual related tasks, from answering simple questions to providing in-depth explanations and discussions on a wide range of topics. CheemsChat is able to generate human-like text based on the input it receives, allowing it to engage in natural-sounding conversations and provide responses that are coherent and relevant to the topic at hand.

CheemsChat is able to process and understand large amounts of text and images. As a language model, CheemsChat can not directly read images, but it has a list of tools to finish different visual tasks. Each image will have a file name formed as "image/xxx.png", and CheemsChat can invoke different tools to indirectly understand pictures. When talking about images, CheemsChat is very strict to the file name and will never fabricate nonexistent files. When using tools to generate new image files, CheemsChat is also known that the image may not be the same as the user's demand, and will use other visual question answering tools or description tools to observe the real image. CheemsChat is able to use tools in a sequence, and is loyal to the tool observation outputs rather than faking the image content and image file name. It will remember to provide the file name from the last tool observation, if a new image is generated.

Human may provide new figures to CheemsChat with a description. The description helps CheemsChat to understand this image, but CheemsChat should use tools to finish following tasks, rather than directly imagine from the description.

Overall, CheemsChat is a powerful visual dialogue assistant tool that can help with a wide range of tasks and provide valuable insights and information on a wide range of topics. 


TOOLS:
------

CheemsChat  has access to the following tools:"""

CHEEMS_CHAT_FORMAT_INSTRUCTIONS = """To use a tool, please use the following format:
...
CHEEMS_CHAT_SUFFIX = """You are very strict to the filename correctness and will never fake a file name if it does not exist.
...
New input: {input}
Since CheemsChat is a text language model, CheemsChat must use tools to observe images rather than imagination.
The thoughts and observations are only visible for CheemsChat, CheemsChat should remember to repeat important information in the final response for Human. 
Thought: Do I need to use a tool? {agent_scratchpad} Let's think step by step.
"""

CHEEMS_CHAT_PREFIX_CN = """CheemsChat 旨在能够协助完成范围广泛的文本和视觉相关任务，从回答简单的问题到提供对广泛主题的深入解释和讨论。 CheemsChat 能够根据收到的输入生成类似人类的文本，使其能够进行听起来自然的对话，并提供连贯且与手头主题相关的响应。

CheemsChat 能够处理和理解大量文本和图像。作为一种语言模型，CheemsChat 不能直接读取图像，但它有一系列工具来完成不同的视觉任务。每张图片都会有一个文件名，格式为“image/xxx.png”，CheemsChat可以调用不同的工具来间接理解图片。在谈论图片时，CheemsChat 对文件名的要求非常严格，绝不会伪造不存在的文件。在使用工具生成新的图像文件时，CheemsChat也知道图像可能与用户需求不一样，会使用其他视觉问答工具或描述工具来观察真实图像。 CheemsChat 能够按顺序使用工具，并且忠于工具观察输出，而不是伪造图像内容和图像文件名。如果生成新图像，它将记得提供上次工具观察的文件名。

Human 可能会向 CheemsChat 提供带有描述的新图形。描述帮助 CheemsChat 理解这个图像，但 CheemsChat 应该使用工具来完成以下任务，而不是直接从描述中想象。有些工具将会返回英文描述，但你对用户的聊天应当采用中文。

总的来说，CheemsChat 是一个强大的可视化对话辅助工具，可以帮助处理范围广泛的任务，并提供关于范围广泛的主题的有价值的见解和信息。
...
CHEEMS_CHAT_FORMAT_INSTRUCTIONS_CN = """用户使用中文和你进行聊天，但是工具的参数应当使用英文。如果要调用工具，你必须遵循如下格式:
...
CHEEMS_CHAT_SUFFIX_CN = """你对文件名的正确性非常严格，而且永远不会伪造不存在的文件。

开始!

因为CheemsChat是一个文本语言模型，必须使用工具去观察图片而不是依靠想象。
推理想法和观察结果只对CheemsChat可见，需要记得在最终回复时把重要的信息重复给用户，你只能给用户返回中文句子。我们一步一步思考。在你使用工具时，工具的参数只能是英文。
...
##################### New SegmentAnything-related Models ##########
class SegmentAnything:
    ...
    def inference(self, inputs):
        ...
        plt.axis('off')
        updated_image_path = get_new_image_name(inputs, func_name="sam")
        plt.savefig(updated_image_path, bbox_inches='tight')
        print(f"\nProcessed SegmentAnything, Input Image: {inputs}, Output Depth: {updated_image_path}")
        return updated_image_path


class PartPromptSegmentAnything:
    def __init__(self, device):
        ...
        self.vlpart.to(device=device)

    @prompts(name="Part Level Segment Based on Given Prompts",
             description="useful when you give prompts at part level and what to segment the part in the image"
                         "like: segment the dog head from this image, "
                         "The input to this tool should be a comma separated string of two, representing the image_path and given part related prompts,"
                         "which shoud only contain noun."
                         )
                         
    def inference(self, inputs):
        print("=====  PartPromptSegmentAnything ====")
        print("Inputs: ", inputs)
        ...
        vl_inputs = {"image": image, "height": height, "width": width}
        with torch.no_grad():
...
#                                         masks.to('cpu'), text_prompt)
        ...
        print(f"\nProcessed PartPromptSegmentAnything, Input Image: {inputs}, Output Depth: {updated_image_path}")
        return updated_image_path


class ObjectPromptSegmentAnything:
    def __init__(self, device):
        print(f"Initializing ObjectPromptSegmentAnything to {device}")
        ...
        glip_checkpoint = "glip_large.pth"
        cfg.local_rank = 0
        cfg.num_gpus = 1
    ...
    @prompts(name="Object Level Segment Based on Given Prompts",
             description="useful when you give prompts at object level and what to segment the whole object in the image"
                         "like: segment the dog from this image, "
                         "The input to this tool should be a comma separated string of two, representing the image_path and given whole object related prompts,"
                         "which shoud only contain noun."
                         )
                         
    def inference(self, inputs):
        print("=====  ObjectPromptSegmentAnything ====")
        print("Inputs: ", inputs)
        image_path, text_prompt = inputs.split(',')
        image = cv2.imread(image_path)

        # model inference
        scores, boxes, names = self.glip_demo.inference_on_image(image, text_prompt)
        masks = None
        ...
        if len(boxes) > 0:
            show_predictions_with_masks_glip(scores, boxes, names, masks.to('cpu'))
        plt.axis('off')
        updated_image_path = get_new_image_name(inputs, func_name="opsam")
        plt.savefig(
            updated_image_path,
            bbox_inches="tight", dpi=300, pad_inches=0.0
        )
        print(f"\nProcessed ObjectPromptSegmentAnything, Input Image: {inputs}, Output Depth: {updated_image_path}")
        return updated_image_path
...
class ConversationBot:
    ...
    def init_agent(self, lang):
        ...
        if lang=='English':
            PREFIX, FORMAT_INSTRUCTIONS, SUFFIX = CHEEMS_CHAT_PREFIX, CHEEMS_CHAT_FORMAT_INSTRUCTIONS, CHEEMS_CHAT_SUFFIX
            place = "Enter text and press enter, or upload an image"
            label_clear = "Clear"
        else:
            PREFIX, FORMAT_INSTRUCTIONS, SUFFIX = CHEEMS_CHAT_PREFIX_CN, CHEEMS_CHAT_FORMAT_INSTRUCTIONS_CN, CHEEMS_CHAT_SUFFIX_CN
...
if __name__ == '__main__':
    ...
    with gr.Blocks(css="#chatbot .overflow-y-auto{height:500px}") as demo:
        ...
        chatbot = gr.Chatbot(elem_id="chatbot", label="CheemsChat")
        state = gr.State([])
